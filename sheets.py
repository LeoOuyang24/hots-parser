...
#returns worksheet we are working with (the first worksheet)
def getWorksheet():
    gc = gspread.service_account("./gspread/service_account.json")

    ws = gc.open("fenchad spreadsheet").get_worksheet(0)
    ...
    if colLength > 1:
        ...
        for key in data[0]: 
            ...
            for i in range(len(data)):
                ...
                #all data cells have their text centered and have padding of 20 pixels (to make the hero pics look okay)
                ...
                elif key == "HERO": #for hero names, grab the hero profile pic from heroesprofile
                    val = "=image(\"https://www.heroesprofile.com/images/heroes/" + stripAccents(val.lower().translate(str.maketrans('', '', string.punctuation))) + ".png\")"
